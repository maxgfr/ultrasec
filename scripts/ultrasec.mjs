...
import { mkdirSync, readdirSync as readdirSync22, readFileSync as readFileSync4, rmSync, statSync as statSync22, writeFileSync as writeFileSync2 } from "fs";
...
function collectCalls(root, spec) {
  ...
  const out2 = [];
  const seen = /* @__PURE__ */ new Set();
  const add2 = (name2, node, receiver) => {
  ...
  const visit = (node) => {
    ...
    if (how === "function") {
      ...
      add2(readName(callee), node, readReceiver(callee) ?? readReceiver(node));
    } else if (how === "member") {
      add2(readName(node.childForFieldName("name")), node, readReceiver(node));
      ...
      add2(readName(t), node, readReceiver(t ?? null));
    }
    for (let i2 = 0; i2 < node.namedChildCount; i2++) visit(node.namedChild(i2));
  };
  visit(root);
...
function scanRepo2(root, opts = {}) {
  ...
  const { files: walked, capped, excluded } = walk2(root, {
...
async function callTool(name2, args2) {
  ...
  if (name2 === "dead_code") {
    return JSON.stringify(findDeadCode(scanRepo2(repo, scanOpts)), null, 2);
  }
  if (name2 === "complexity") {
    const scan2 = scanRepo2(repo, scanOpts);
    if (args2.risk === true) {
...
async function runCli(argv) {
  ...
  if (cmd === "index") {
    ...
    const cachePath2 = join12(outDir, "cache.json");
    let cache;
    try {
      const parsed = JSON.parse(readFileSync6(cachePath2, "utf8"));
...
`);
    ...
    emit(JSON.stringify(findDeadCode(scanRepo2(flags2.repo, scanOptions(flags2))), null, 2) + "\n", flags2.out);
...
// src/store.ts
import { mkdirSync as mkdirSync3, writeFileSync as writeFileSync4, readFileSync as readFileSync7, existsSync as existsSync5 } from "fs";
...
function writeDossier(outDir, d) {
  ...
  writeFileSync4(join13(outDir, "manifest.json"), JSON.stringify(d.manifest, null, 2));
  writeFileSync4(join13(outDir, "findings.json"), JSON.stringify(d.findings, null, 2));
  writeFileSync4(join13(outDir, "graph.json"), JSON.stringify(d.graph, null, 2));
  writeFileSync4(join13(outDir, "DOSSIER.md"), renderDossierMd(d));
}
...
function loadDossier(outDir) {
  const read = (name2) => JSON.parse(readFileSync7(join13(outDir, name2), "utf8"));
  if (!existsSync5(join13(outDir, "findings.json"))) {
...
import { mkdirSync as mkdirSync4, writeFileSync as writeFileSync5, readFileSync as readFileSync8, existsSync as existsSync6 } from "fs";
...
function buildAttackSurface(scan2, coveredScopes = []) {
  ...
  for (const f of scan2.files) {
    ...
    la.files++;
    da.files++;
    const sources = findSources(lang, readText(join14(scan2.repo, f.rel)));
...
async function runMap(args2) {
  ...
  let coveredScopes = [];
  if (out2) {
    const mPath = join15(resolve4(out2), "manifest.json");
    if (existsSync6(mPath)) {
      try {
        const m = JSON.parse(readFileSync8(mPath, "utf8"));
  ...
  if (out2) {
    ...
    writeFileSync5(join15(outDir, "attack-surface.json"), JSON.stringify(surface, null, 2));
    writeFileSync5(join15(outDir, "MAP.md"), renderMapMd(repo, surface));
  }
  if (flagBool(args2, "json")) {
  ...
  if (out2) println(`
wrote ${join15(resolve4(out2), "MAP.md")} + attack-surface.json`);
  return 0;
}

// src/commands/scan.ts
import { resolve as resolve5, join as join25, relative as relative2 } from "path";
...
function enumerateTaint(scan2, graph, opts = {}) {
  ...
  const content = (rel) => {
    ...
    if (c2 === void 0) contentCache.set(rel, c2 = readText(join16(scan2.repo, rel)));
    return c2;
  };
  const lines = (rel) => {
...
function enumerateSinkCandidates(scan2, covered, opts = {}) {
  ...
  const lines = (rel) => {
    let l = lineCache.get(rel);
    if (!l) lineCache.set(rel, l = readText(join17(scan2.repo, rel)).split(/\r?\n/));
    return l;
  };
  const findings = [];
  for (const file of scan2.files) {
...
function loadCodeowners(repo) {
  for (const p of CODEOWNERS_PATHS) {
    const abs = join18(repo, p);
    if (existsSync7(abs)) {
      try {
        return parseCodeowners(readFileSync9(abs, "utf8"));
      } catch {
        return [];
      }
    }
...
// src/cache.ts
import { mkdirSync as mkdirSync5, writeFileSync as writeFileSync6, readFileSync as readFileSync10 } from "fs";
import { join as join19 } from "path";
var CACHE_VERSION = 1;
function cachePath(run2) {
  return join19(run2, "cache", "scan-cache.json");
}
function loadScanCache(run2) {
  try {
    const data = JSON.parse(readFileSync10(cachePath(run2), "utf8"));
...
function saveScanCache(run2, cache) {
  const dir = join19(run2, "cache");
  mkdirSync5(dir, { recursive: true });
  const entries = {};
  for (const [k, v] of [...cache.entries()].sort((a, b) => byStr(a[0], b[0]))) entries[k] = v;
  writeFileSync6(cachePath(run2), JSON.stringify({ cacheVersion: CACHE_VERSION, entries }, null, 2));
}
...
import { existsSync as existsSync8, mkdirSync as mkdirSync6, readFileSync as readFileSync11, statSync as statSync4, writeFileSync as writeFileSync7 } from "fs";
...
function cacheDir() {
  return process.env.ULTRASEC_CACHE_DIR || join20(homedir(), ".cache", "ultrasec");
}
function fresh(path) {
  try {
    return existsSync8(path) && Date.now() - statSync4(path).mtimeMs < TTL_MS;
  } catch {
    return false;
  }
}
...
async function loadCached(url, file, gz) {
  ...
  try {
    ...
    try {
      if (existsSync8(path)) return readFileSync11(path, "utf8");
    } catch {
    }
    return "";
  }
...
var gitleaks = {
  ...
  argv: (target) => {
    ...
    const hasGit = onHost && existsSync9(join21(target, ".git"));
...
function ghsaFromUrl(url) {
  ...
  return m ? m[0].toUpperCase() : void 0;
}
...
function npmV6AdvisoryFinding(tool, id, a, lockfile) {
...
function parseNpmV6Advisories(data, lockfile, tool) {
  ...
  for (const id of Object.keys(advisories)) {
    ...
    if (!a) continue;
    out2.push(npmV6AdvisoryFinding(tool, id, a, lockfile));
  }
  return out2;
}
function parseNpmV7(data, lockfile) {
...
function npmLockfileName(repo) {
  if (!existsSync11(join24(repo, "package-lock.json")) && existsSync11(join24(repo, "npm-shrinkwrap.json"))) return "npm-shrinkwrap.json";
...
var npmAudit = {
  ...
  applicable: (repo) => existsSync11(join24(repo, "package-lock.json")) || existsSync11(join24(repo, "npm-shrinkwrap.json")) ? null : "no package-lock.json",
  argv: () => ["audit", "--json"],
  parse(raw, repo) {
...
var pnpmAudit = {
  ...
  applicable: (repo) => existsSync11(join24(repo, "pnpm-lock.yaml")) ? null : "no pnpm-lock.yaml",
  argv: () => ["audit", "--json"],
  parse(raw) {
    const data = parseJson(raw);
    if (!data || typeof data !== "object") return [];
...
function yarnMajor() {
  ...
  try {
    const out2 = execFileSync4("yarn", ["--version"], { stdio: ["ignore", "pipe", "ignore"], timeout: 5e3 }).toString().trim();
...
function yarnBerryFinding(entry) {
  ...
  const ghsa = ghsaFromUrl(c2.URL);
  const cves = cvesIn(c2.Issue, c2.URL);
  const aliases = [ghsa, ...cves].filter(Boolean);
  const treeVersions = c2["Tree Versions"];
  const version = Array.isArray(treeVersions) ? treeVersions[0] : void 0;
  const ident = String(c2.ID ?? ghsa ?? cves[0] ?? pkg);
  const vulnerable = c2["Vulnerable Versions"];
  return makeToolFinding({
    tool: "yarn-audit",
    category: "dep",
    ident,
...
var yarnAudit = {
  ...
  network: true,
  streaming: true,
  applicable: (repo) => existsSync11(join24(repo, "yarn.lock")) ? null : "no yarn.lock",
  command: () => {
...
async function runScan(args2) {
  ...
  if (diffRef) {
    ...
    let targets = changed;
    if (existsSync12(join25(out2, "graph.json"))) {
      try {
  ...
  let mergedNote = "";
  if (flagBool(args2, "merge") && existsSync12(join25(out2, "findings.json"))) {
    try {
...
function detectFrameworks(repo) {
  const found = /* @__PURE__ */ new Set();
  const pkgPath = join26(repo, "package.json");
...
function buildContextScaffold(repo, scan2, surface) {
  ...
  for (const fileScan of scan2.files) {
    ...
    if (!spec) continue;
    const lines = readText(join26(repo, fileScan.rel)).split(/\r?\n/);
    for (let i2 = 0; i2 < lines.length; i2++) {
      const line = lines[i2];
...
function renderContextScaffoldMd(repo, run2, s) {
  ...
  L.push(`- repo: \`${repo}\``);
  L.push("");
  L.push(`> The deterministic scaffold below is a STARTING POINT. Author **\`${join26(run2, "CONTEXT.md")}\`**`);
...
function loadContextDoc(run2) {
  const p = join26(run2, "CONTEXT.md");
...
function runContext(args2) {
  ...
  writeFileSync8(join27(out2, "CONTEXT.scaffold.json"), JSON.stringify(scaffold, null, 2));
  writeFileSync8(join27(out2, "CONTEXT.todo.md"), renderContextScaffoldMd(repo, out2, scaffold));
  if (flagBool(args2, "json")) {
    println(JSON.stringify(scaffold, null, 2));
    return 0;
  }
  println(`ultrasec context \u2192 ${out2}`);
  println(`  ${join27(out2, "CONTEXT.scaffold.json")}  \xB7  ${join27(out2, "CONTEXT.todo.md")}`);
  println(
  ...
  println(`  next: author ${join27(out2, "CONTEXT.md")} (see CONTEXT.todo.md), then run \`scan\`/\`verify\` \u2014 it's injected into every dossier.`);
  return 0;
}

// src/commands/import.ts
import { resolve as resolve7, join as join28 } from "path";
...
async function runImport(args2) {
  ...
  let raw;
  try {
    raw = readFileSync13(resolve7(file), "utf8");
  } catch (e) {
  ...
  let prev;
  if (existsSync14(join28(run2, "findings.json"))) {
    try {
...
function excerpt(repo, step, ctx = 3) {
  const lines = readText(join29(repo, step.file)).split(/\r?\n/);
...
function emitWorklist(run2, files, items, md) {
  ...
  const todoPath = join30(run2, files.todo);
  writeFileSync9(todoPath, JSON.stringify(items, null, 2));
  writeFileSync9(join30(run2, files.md), md);
  return todoPath;
}
function collectApplyFiles(applyPath, dirRegex) {
  ...
  if (isDir) {
    const matches = readdirSync4(abs).filter((n) => dirRegex.test(n)).sort().map((n) => join30(abs, n));
...
function readApply(applyPath, dirRegex, parse) {
  ...
  for (const f of collectApplyFiles(applyPath, dirRegex)) {
    try {
      out2.push(...parse(readFileSync14(f, "utf8")));
    } catch (e) {
...
import { join as join31, resolve as resolve11, sep as sep3 } from "path";
...
function lineCount(repo, file) {
  ...
  const abs = join31(repo, file);
  if (!existsSync15(abs)) return null;
  try {
...
import { join as join32, resolve as resolve14 } from "path";
...
function applyMode(run2, dossier, applyPath, args2) {
  ...
  println(`ultrasec verify --apply \u2192 updated ${join32(run2, "findings.json")}`);
...
function loadNarrative(run2, dossier, file) {
  const p = file ?? join33(run2, "NARRATIVE.json");
...
function runRender(args2) {
  ...
  if (narrativePath) {
    let parsed;
    try {
      parsed = parseNarrative(readFileSync17(resolve19(narrativePath), "utf8"));
    } catch (e) {
  ...
  for (const [name2, body2] of outputs) writeFileSync10(join34(run2, name2), body2);
  println(`ultrasec render \u2192 ${run2}`);
  for (const [name2] of outputs) println(`  ${join34(run2, name2)}`);
...
import { existsSync as existsSync17, rmSync as rmSync2, readdirSync as readdirSync5 } from "fs";
import { join as join35, resolve as resolve20 } from "path";
...
function dockerAvailable() {
  try {
    execFileSync5("docker", ["--version"], { stdio: "ignore", timeout: 5e3 });
    return true;
  } catch {
    return false;
  }
...
function docker(args2) {
  try {
    const out2 = execFileSync5("docker", args2, { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"], timeout: 6e4 });
...
function runClean(args2) {
  ...
  const removed = [];
  const kept = [];
  if (!keepOutput && existsSync17(run2)) {
    if (all) {
      ...
      for (const entry of readdirSync5(run2)) {
        if (DELIVERABLES.has(entry)) {
          preservedAny = true;
          kept.push(`deliverable  ${join35(run2, entry)}`);
          continue;
        }
        if (!dry) rmSync2(join35(run2, entry), { recursive: true, force: true });
        removed.push(`intermediate  ${join35(run2, entry)}`);
      }
      if (!preservedAny) {
        if (!dry) rmSync2(run2, { recursive: true, force: true });
      }
...
import { join as join37, resolve as resolve21 } from "path";
...
function nonEmptyFile(p) {
  try {
    return existsSync18(p) && statSync6(p).size > 0;
  } catch {
    return false;
  }
}
...
var STAGES = {
  context: {
    ...
    emit(repo, run2) {
      ...
      writeFileSync11(join36(run2, "CONTEXT.scaffold.json"), JSON.stringify(scaffold, null, 2));
      const wl = join36(run2, "CONTEXT.todo.md");
  ...
  triage: {
    ...
    emit(repo, run2, dossier) {
      ...
      return { worklist: join36(run2, f.md), outName: "TRIAGE.json" };
    },
  ...
  investigate: {
    ...
    emit(repo, run2, dossier) {
      ...
      return { worklist: join36(run2, f.md), outName: "INVESTIGATE.json" };
    },
  ...
  verify: {
    ...
    emit(repo, run2, dossier) {
      ...
      return { worklist: join36(run2, f.md), outName: "verdicts.json" };
    },
  ...
  revalidate: {
    ...
    emit(repo, run2, dossier) {
      ...
      return { worklist: join36(run2, f.md), outName: "REVALIDATE.json" };
    },
  ...
  narrative: {
    ...
    emit(repo, run2, dossier) {
      ...
      return { worklist: join36(run2, f.md), outName: "NARRATIVE.json" };
    },
  ...
  implement: {
    ...
    emit(repo, run2, dossier) {
      ...
      return { worklist: join36(run2, f.md), outName: "REMEDIATION_PRD.md" };
    },
...
function runPipeline(opts) {
  ...
  for (const name2 of opts.stages) {
    ...
    const outPath = join36(opts.run, outName);
    ...
    if (opts.crossRunner && stage.crossCheckable) {
      const crossPath = join36(opts.run, `${outName}.cross.json`);
      ...
      externalCalls++;
      if (cr.ok) {
        const cross = stage.applyPure(opts.repo, opts.run, after, readFileSync18(crossPath, "utf8"));
  ...
  actions.push("check");
  let narrative;
  const narrPath = join36(opts.run, "NARRATIVE.json");
  ...
  writeFileSync11(join36(opts.run, "SUMMARY.md"), renderSummary(dossier, narrative));
  writeFileSync11(join36(opts.run, "REPORT.md"), renderReport(dossier, narrative));
  writeFileSync11(join36(opts.run, "index.html"), renderHtml(dossier, narrative));
  actions.push("render");
...
function runRun(args2) {
  ...
  if (noScan && !existsSync19(join37(run2, "findings.json"))) {
  ...
  if (!powered) {
    ...
    for (const e of res.emitted) {
      ...
      println(`    - ${e.stage}: read ${e.worklist}, write ${join37(run2, e.outName)}${apply}`);
    }
  ...
  println(`  report: ${join37(run2, "REPORT.md")} \xB7 ${join37(run2, "index.html")}`);
...
// src/orchestrate.ts
import { existsSync as existsSync20, mkdirSync as mkdirSync9, readFileSync as readFileSync19, writeFileSync as writeFileSync12 } from "fs";
import { join as join39, resolve as resolve22 } from "path";
...
var PHASE_SPECS = {
  adjudicate: {
    ...
    applyHint: (engine, _worklist, run2) => `node ${engine} verify --apply ${join38(run2, "orchestration", "out", "adjudicate", "verdicts.json")} --run ${run2}`,
    fragmentFile: (run2) => join38(run2, "orchestration", "out", "adjudicate", "verdicts.json")
  },
  verify: {
    role: "skeptic",
    title: "Verify",
    ...
    applyHint: (engine, _worklist, run2) => `node ${engine} verify --apply ${join38(run2, "orchestration", "out", "verify", "verdicts.json")} --run ${run2}`,
    fragmentFile: (run2) => join38(run2, "orchestration", "out", "verify", "verdicts.json")
  },
  revalidate: {
    role: "revalidator",
    title: "Revalidate",
    ...
    applyHint: (engine, _worklist, run2) => `node ${engine} revalidate --apply ${join38(run2, "orchestration", "out", "revalidate", "REVALIDATE.json")} --run ${run2}`,
    fragmentFile: (run2) => join38(run2, "orchestration", "out", "revalidate", "REVALIDATE.json")
  },
  investigate: {
    role: "hunter",
    title: "Investigate",
    ...
    applyHint: (engine, _worklist, run2) => `node ${engine} investigate --apply ${join38(run2, "orchestration", "out", "investigate", "INVESTIGATE.json")} --run ${run2}`,
    fragmentFile: (run2) => join38(run2, "orchestration", "out", "investigate", "INVESTIGATE.json")
  }
};
...
function phaseWorkflowScript(ph, runAbs, engineAbs, batchSize) {
  ...
  const scriptPath = join38(runAbs, "orchestration", `${ph.name}.workflow.mjs`);
...
Worklist: \`${join38(runAbs, "findings.json")}\` (the audit dossier's candidate list; repo root: \`${repoAbs}\`). Handle ONLY the findings whose \`id\` is named in your prompt (\`ITEMS=<id,\u2026>\`). If an \`ITEMS\` id is no longer in the worklist, skip it and say so in your note.
...
Worklist: \`${join38(runAbs, "VERIFY.todo.json")}\` (a JSON array; each entry has \`id\`, \`severity\`, \`cwe\`, \`title\`, \`category\`, \`claim\`, \`files[]\`; repo root: \`${repoAbs}\`). Handle ONLY the entries whose \`id\` is named in your prompt (\`ITEMS=<id,\u2026>\`). If an \`ITEMS\` id is no longer in the worklist, skip it and say so in your note.
...
Worklist: \`${join38(runAbs, "INVESTIGATE.todo.json")}\` (a JSON array; each entry has \`region\`, \`files[]\`, \`neighbors[]\`, \`prompt\`; paths are relative to the repo root \`${repoAbs}\`). Handle ONLY the regions named in your prompt (\`ITEMS=<region,\u2026>\`). If an \`ITEMS\` region is no longer in the worklist, skip it and say so in your note.
...
function runbookMd(phases, runAbs, engineAbs, repoAbs) {
  ...
  const agents = (role) => join38(runAbs, "orchestration", "agents", `${role}.md`);
...
1. **Scan** (if not done): \`${engine} scan --repo ${repoAbs} --out ${runAbs}\` \u2192 \`${join38(runAbs, "findings.json")}\` (+ optionally prime \`${engine} context\`).
2. **Investigate the attack surface** (discovery) \u2014 \`${engine} investigate --run ${runAbs}\` writes \`${join38(runAbs, "INVESTIGATE.todo.json")}\`. For EVERY region, apply \`${agents("hunter")}\` yourself; merge the grounded Discovery[] into \`${frag("investigate")}\`. Then ingest (citation-checked): \`${phaseSpec("investigate").applyHint(engineAbs, "", runAbs)}\`.
3. **Adjudicate the open candidates** \u2014 the worklist is \`${join38(runAbs, "findings.json")}\` itself (every \`status: "open"\` candidate). For EVERY open id, apply \`${agents("analyzer")}\` yourself (\`${engine} dossier <id> --run ${runAbs}\`, read every hop, verdict supported/partial/unsupported/refuted + note, exploitPath when supported); merge the verdicts into \`${frag("adjudicate")}\`. Then fold, conservatively: \`${phaseSpec("adjudicate").applyHint(engineAbs, "", runAbs)}\`.
4. **Verify adversarially** \u2014 \`${engine} verify --run ${runAbs}\` writes \`${join38(runAbs, "VERIFY.todo.json")}\` (the still-pending findings). For EVERY entry, apply \`${agents("skeptic")}\` yourself (try to REFUTE; uncertain high-severity stays needs-human); merge into \`${frag("verify")}\`. Then: \`${phaseSpec("verify").applyHint(engineAbs, "", runAbs)}\`.
5. **Revalidate against git history** \u2014 \`${engine} revalidate --run ${runAbs}\` writes \`${join38(runAbs, "REVALIDATE.todo.json")}\`. For EVERY entry, apply \`${agents("revalidator")}\` yourself (still-valid/fixed/false-positive/uncertain + note, fixedIn when fixed); merge into \`${frag("revalidate")}\`. Then: \`${phaseSpec("revalidate").applyHint(engineAbs, "", runAbs)}\`.
...
With subagents available, prefer the emitted workflows instead: \`orchestrate --run ${runAbs} --phase <p>\` then \`Workflow({ scriptPath: "${join38(runAbs, "orchestration", "<p>.workflow.mjs")}" })\` \u2014 you stay the sole writer either way.
`;
}
...
function listPhases(runDir, engineAbs) {
  ...
  const findingsPath = join39(run2, "findings.json");
  const allIds = readIds(findingsPath, (f) => f.id);
  let adjIds = [];
  if (allIds !== null) {
    try {
      const findings = JSON.parse(readFileSync19(findingsPath, "utf8"));
      adjIds = findings.filter((f) => f.status === "open").map((f) => f.id);
    } catch {
    }
  }
  const verPath = join39(run2, "VERIFY.todo.json");
  const verIds = readIds(verPath, (i2) => i2.id);
  const revPath = join39(run2, "REVALIDATE.todo.json");
  const revIds = readIds(revPath, (i2) => i2.id);
  const invPath = join39(run2, "INVESTIGATE.todo.json");
...
function repoOf(run2) {
  try {
    const m = JSON.parse(readFileSync19(join39(run2, "manifest.json"), "utf8"));
...
function orchestrateRun(runDir, engineAbs, opts = {}) {
  ...
  const orchDir = join39(run2, "orchestration");
  const agentsDir = join39(orchDir, "agents");
  for (const p of PHASES) mkdirSync9(join39(orchDir, "out", p), { recursive: true });
  mkdirSync9(agentsDir, { recursive: true });
  const written = [];
  const notices = [];
  for (const [name2, content] of Object.entries(agentContracts(run2, engineAbs, repoAbs))) {
    const p = join39(agentsDir, `${name2}.md`);
  ...
  if (!opts.eco) {
    for (const ph of selected) {
      ...
      const p = join39(orchDir, `${ph.name}.workflow.mjs`);
      writeFileSync12(p, phaseWorkflowScript(ph, run2, engineAbs, BATCH_SIZE));
      written.push(p);
    }
  }
  const rb = join39(orchDir, "RUNBOOK.md");
...
function runOrchestrate(args2) {
  ...
  if (workflows.length) {
    ...
    println(`Follow ${join40(runFlag, "orchestration", "RUNBOOK.md")} sequentially (the eco path).`);
  }
