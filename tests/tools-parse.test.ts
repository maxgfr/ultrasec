...
describe("cargo-audit adapter", () => {
  ...
  // MINOR 8: gate on Cargo.lock (pip-audit's requirements.txt pattern) so a
  // non-Rust repo gets a clean skip instead of a noisy "run failed".
  it("applicable(): null (run) when Cargo.lock exists, a skip note otherwise", () => {
    const withLock = mkdtempSync(join(tmpdir(), "ultrasec-cargo-audit-"));
    writeFileSync(join(withLock, "Cargo.lock"), "# This file is automatically @generated by Cargo.\nversion = 3\n");
